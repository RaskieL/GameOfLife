...
class World:
    ...
    grid: List[List[Union[Particle, Star]]]
    ...
    def __init__(self, p_width: int, p_height: int, p_size: int) -> None:
        ...
        self.grid = [[Particle(False, x, y) for x in range(self.cols)] for y in range(self.rows)]
    ...
    def init_world(self) -> None:
        ...
        star1: Star = Star(2, 4, random.randint(0, self.cols-1), random.randint(0, self.rows-1))
        ...
        self.stars.extend([star1, star2, star3])

        for s in self.stars:
            self.place_star(s, int(s.pos_x), int(s.pos_y)) # Aseguramos que pasamos int

        total: int = self.cols * self.rows
        print(f"Alive cells: {alive}, Dead cells (yes, like the game): {total - alive}, Percentage alive: {alive / total * 100:.2f}%")

    def handle_star_physics(self) -> None:
        for i, s1 in enumerate(self.stars):
            ...
            for j, s2 in enumerate(self.stars):
                ...
                if dist < 10: dist = 10.0
                if dist_sq == 0: dist_sq = 1.0

                force: float = s2.mass / dist_sq
                s1.ax += (dx / dist) * force
                s1.ay += (dy / dist) * force
            ...
            if s1.x < 0 or s1.x >= self.cols:
                s1.vx *= -0.7
                s1.x = max(0.0, min(s1.x, float(self.cols - 1)))
            
            if s1.y < 0 or s1.y >= self.rows:
                s1.vy *= -0.7
                s1.y = max(0.0, min(s1.y, float(self.rows - 1)))

            # Actualizamos posición entera para la grid
            s1.pos_x = int(s1.x)
            s1.pos_y = int(s1.y)
    ...
    def handle_particle_movement(self) -> None:
        ...
        for y in range(self.rows):
            for x in range(self.cols):
                ...
                for star in self.stars:
                    dx: float = star.x - particle.x
                    dy: float = star.y - particle.y
                    dist_sq: float = dx*dx + dy*dy
                    dist: float = math.sqrt(dist_sq)

                    if dist < 10: dist = 10.0
                    
                    force: float = star.mass / dist_sq
                ...
                nx: int = int(particle.x)
                ny: int = int(particle.y)
        ...
        for s in self.stars:
            self.place_star(s, int(s.pos_x), int(s.pos_y))
